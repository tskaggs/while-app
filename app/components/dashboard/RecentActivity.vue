...
<template>
  <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
    <UCard>
      <template #header>
        <div class="flex items-center justify-between">
          <h3 class="font-semibold text-highlighted">Connection Health</h3>
          <UButton to="/connections" label="View all" color="neutral" variant="ghost" size="xs" />
        </div>
      </template>
      ...
      <div class="space-y-3">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-highlighted truncate">{{ conn.partnerName }}</p>
            <p class="text-xs text-muted">{{ conn.ehrVendor }} · {{ conn.messagesProcessed24h.toLocaleString() }} msgs/24h</p>
          </div>
    ...
    <UCard>
      <template #header>
        <div class="flex items-center justify-between">
          <h3 class="font-semibold text-highlighted">Recent Support Requests</h3>
          <UButton to="/support/request" label="New request" color="neutral" variant="ghost" size="xs" icon="i-lucide-plus" />
        </div>
      </template>
      ...
      <div class="space-y-3">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-highlighted truncate">{{ req.partnerName }}</p>
            <p class="text-xs text-muted">Target: {{ formatDate(req.targetGoLive) }}</p>
          </div>
